...
data_path   <- file.path(dirname(script_path), "data")
export_path <- file.path(dirname(script_path), "export")
...
all_data <- data.table::fread(file.path(data_path, "edw_cdr.csv"))
...
test_preds <- cbind(all_data[ind == "Test"],
                    data.table::as.data.table(h2o.predict(object = best_model$model,
